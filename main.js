...
// ─────────────────────────────────────────────
//  📦  IMPORTS — HELPERS
//  ⚠️  handleTicTacToeMove retiré de helpers.js
//      → remplacé par le nouveau handler ci-dessous
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
//  🎮  IMPORTS — TICTACTOE (nouveau système)
// ─────────────────────────────────────────────
...
const {
    ...
    scoreCommand,
    rankingCommand,
} = require('./commands/game/tictactoe.js');
...
const {
    ...
    callGPT5,
    callMetaAI
} = require('./commands/ia/ai.js');

// ✅ handleChatbotResponse importé depuis chatbot.js
...
const { handleDownloadCommand } = require('./commands/datas/download.js');
...
/**
 * Vérifie si un message est un coup TicTacToe valide (1-100)
 * ou une commande d'abandon.
 */
function isTicTacToeInput(msg) {
    if (/^(quit|abandon|forfait|gg)$/i.test(msg.trim())) return true;
...
async function handleMessages(sock, messageUpdate, printLog) {
    ...
    try {
        ...
        // ── Traduction de préfixe ─────────────────────────────────
        const botNumber = sock.user?.id?.split(':')[0];
        const currentPrefix = (global.userPrefixes && global.userPrefixes[botNumber])
            ? global.userPrefixes[botNumber]
            : '#';
        ...
        // ── Coups TicTacToe (1-100, quit, abandon...) ────────────
        // Prioritaire sur tout le reste pour ne pas bloquer les parties
        if (isTicTacToeInput(userMessage)) {
        ...
        switch (true) {
            ...
            case userMessage.startsWith('#autoresponse'):
                await autoreponse(sock, chatId, message, userMessage);
                commandExecuted = true;
                break;



            // ── TÉLÉCHARGEMENT (#play, #ytmp3, #ytmp4, #tiktok, #ig) ──
            case userMessage.startsWith('#play') ||
                userMessage.startsWith('#ytmp3') ||
                userMessage.startsWith('#ytmp4') ||
                userMessage.startsWith('#tiktok') ||
                userMessage.startsWith('#ig'): {
                    await handleDownloadCommand(sock, message, currentPrefix);
                    commandExecuted = true;
                    break;
                }
            ...
            case userMessage === '#ttt' ||
                userMessage === '#tictactoe' ||
                userMessage.startsWith('#ttt ') ||
                userMessage === '#ttthelp':
                await tictactoeCommand(
                    sock,
                    chatId,
                    senderId,
                    userMessage.replace(/^#ttt(ictactoe)?\s*/i, '').trim()
                );
                commandExecuted = true;
                break;

            case userMessage === '#score':
                await scoreCommand(sock, chatId, senderId);
                commandExecuted = true;
                break;

            case userMessage === '#ranking' || userMessage === '#classement':
                await rankingCommand(sock, chatId);
                commandExecuted = true;
                break;
